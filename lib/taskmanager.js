...
TaskManager.prototype.log = function Log(name, flag, deps, str){

    var useFlag = false, fn;

    if(typeof flag === 'string' && /^--?[^-]+/.test(flag)){
        useFlag = flag;
        if(typeof deps === 'string' || typeof deps === 'function'){
            str = deps;
            deps = [];
        }
    }else if(typeof deps === 'string' && /^--?[^-]+/.test(deps)){
        useFlag = deps;
        deps = flag;
        flag = useFlag;
    }
    ...
    if(typeof str === 'string'){
        ...
        fn = cc(function * (){
            try{
                var val = yield str.apply(this, arguments);
    ...
    this._taskList.push({
        ...
        name: name,
        flag: flag,
        useFlag: useFlag ? flag.replace(/^-+/, '') : false,
        priority: 1
    });
